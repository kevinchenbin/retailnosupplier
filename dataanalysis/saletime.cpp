...
AnsiString sql;
sql = "select '零售' R0, sum(BS_RetailNoteHeader.TotalDiscountedprice) as R1 ,sum(BS_RetailNoteHeader.TotalFixedprice) as R2,sum(BS_RetailNoteHeader.Totalamount) as R3,sum((CASE WHEN dbo.BS_RetailNoteHeader.TotalDiscountedPrice > (RecCash + RecCardMoney) ";
...
sql =sql + " union all ";
sql =sql + " select '批销' R0 ,sum(BS_WsaleNoteHeader.Checked) as R4 , sum(BS_WsaleNoteHeader.Nochecked) as R5,sum(BS_WsaleNoteHeader.totalamount) as R3,sum(BS_WsaleNoteHeader.discountedprice) as R1,sum(BS_WsaleNoteHeader.fixedprice) as R2  ";
...
sql=sql+" union all ";
sql=sql+"    select '批销结帐' R0,sum(nochecked) R5,sum(totalfixedprice) R2,sum(totalamount) R3,sum(totaldiscountedprice) R1,sum(thistime) R4  from dbo.FN_CUSEndMoneyDocuments a join FN_CUSEndMoneyDtails b on ";
sql=sql+" b.endmoneydocumentid= a.moneycode ";
sql=sql +" where a.state = 1 and clearingtype <>'现场收款' and ('"+DateToStr(dtpstart->Date)+"' is null or datediff(day,'"+DateToStr(dtpstart->Date)+"',date) >= 0) and ('"+DateToStr(dtpend->Date)+"' is null or datediff(day,'"+DateToStr(dtpend->Date)+"',date) <= 0)";
