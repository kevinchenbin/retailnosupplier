...
void __stdcall xiaoshou(TApplication* app, TADOConnection* con,int stgid,bool visale)
{
		Application = app;
                Tfrmsaletime *frm = new Tfrmsaletime(app,con,stgid);

                frm->searchPower =  visale;
                frm->init();
		//frm->Disc = Discstr;
		frm->Show();
}
