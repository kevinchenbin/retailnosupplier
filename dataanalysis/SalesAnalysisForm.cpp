...
void __fastcall TfrmSalesAnalysis::RzToolButton1Click(TObject *Sender)
{
	...
	case 0:   //单品
		...
		if (cbstart->Checked ) {
			sql = sql + " and datediff(d,WsaleTime,'" + DateToStr(dtp1->Date) + "')<=0";
		}
		if (cbend->Checked ) {
			sql = sql + " and datediff(d,WsaleTime,'" + DateToStr(dtp2->Date) + "')>=0";
		}
		sql = sql + " group by BookName,Price,ISBN order by WsaleAmount asc ";
	break;
	case 1:   //类别
		...
		sql = sql + " group by TypeName order by WsaleAmount asc ";
	break;
	case 2: //出版社
		...
		sql = sql + " group by PressName order by WsaleAmount asc ";
	break;
	case 3:   //供应商
		...
		sql = sql + " group by suppliername order by WsaleAmount asc ";
	break;
	default:
		;
	}
