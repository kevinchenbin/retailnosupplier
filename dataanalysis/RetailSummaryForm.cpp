...
void __fastcall TfrmRetailSummary::RzToolButton1Click(TObject *Sender)
{
	...
	if (rg1->ItemIndex == 0) {
		if (rg2->ItemIndex == 0) {  //入库
			...
			sql = "select SYS_StorageInfo.name as storagename,convert(varchar(10),hdtime,120) as date ,sum(A.FixedPrice) fix,sum(A.DiscountedPrice) dis,sum(A.TotalAmount) amount,sum(A.TotalAmount*B.Cbprice) as Cbprice,sum(A.DiscountedPrice-A.TotalAmount*B.Cbprice) as profite"
			...
			sql = "select SYS_StorageInfo.name as storagename,convert(varchar(10),hdtime,120) as date ,sum(FixedPrice) fix,sum(DiscountedPrice) dis,sum(A.Amount) amount,sum(A.Amount*B.Cbprice) as Cbprice,sum(DiscountedPrice-A.Amount*B.Cbprice) as profite"
	...
	//月
		...
		if (rg2->ItemIndex == 0) {  //入库
			sql = "select SYS_StorageInfo.name as storagename,convert(varchar(7),hdtime,120) as date ,sum(FixedPrice) fix,sum(DiscountedPrice) dis,sum(A.Amount) amount,0 as Cbprice,0 as profite "
			...
			" group by convert(varchar(7),hdtime,120),SYS_StorageInfo.name";
		}
		else if (rg2->ItemIndex == 1) { //发货
			sql = "select SYS_StorageInfo.name as storagename,convert(varchar(7),hdtime,120) as date ,sum(A.FixedPrice) fix,sum(A.DiscountedPrice) dis,sum(A.TotalAmount) amount,sum(A.TotalAmount*B.Cbprice) as Cbprice,sum(A.DiscountedPrice-A.TotalAmount*B.Cbprice) as profite"
			" from BS_WsaleNote A left join BS_WsaleNoteHeader on  A.WsaleNtHeaderID = BS_WsaleNoteHeader.id left join STK_BookInfo B on A.BkInfoID = B.ID left join SYS_StorageInfo on BS_WsaleNoteHeader.stgid = SYS_StorageInfo.id where 1=1 " + sqlws +
			" group by convert(varchar(7),hdtime,120),SYS_StorageInfo.name";
		}
		else if (rg2->ItemIndex == 2) { //零售
			sql = "select SYS_StorageInfo.name as storagename,convert(varchar(7),hdtime,120) as date ,sum(FixedPrice) fix,sum(DiscountedPrice) dis,sum(A.Amount) amount,sum(A.Amount*B.Cbprice) as Cbprice,sum(DiscountedPrice-A.Amount*B.Cbprice) as profite"
			" from BS_RetailNote A left join BS_RetailNoteHeader on  A.RetailNtHeaderID = BS_RetailNoteHeader.id left join STK_BookInfo B on A.BkInfoID = B.ID left join SYS_StorageInfo on BS_RetailNoteHeader.stgid = SYS_StorageInfo.id where 1=1 " + sqlls +
			" group by convert(varchar(7),hdtime,120),SYS_StorageInfo.name";
		}
	}
