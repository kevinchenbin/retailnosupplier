...
void __fastcall Tfrmhuancard::BtnSendtoBackClick(TObject *Sender)
{
	...
	if (edtcardID->Text == "") {
                ...
                ShowMsg(Handle ,"请输入新会员卡的卡号！",3);
		return;
	}
	if (edtcardID->Text.Length() != 13 ) {

                ShowMsg(Handle ,"请输入13位的会员卡号！",3);
		return;
	}

	if(ShowMsg(Handle ,"确认换卡！",1)!=1)
	{
		return;
	}
	AnsiString sql;
	...
	if (aq->RecordCount > 0) {
				  ...
				  ShowMsg(Handle ,"该会员卡号已存在，请重新输入会员卡卡号！",3);
		return;
	}
	if (edCardCast->Text == "") {
	...
	sql = "insert into CUST_MemberInfo(Name,Birthday,Sex,Tel,Mobile,Address,PostalCode,CardID,Balance,TotalConsumption,MemberType,CardSN,Memberdot,CardType,PassWord,CardCast,bk,bircard,youxiaodate) values ('" +
			...
			edjifen->Text + "," + IntToStr(rg->ItemIndex) + ",'" + edtpwd->Text + "'," + edCardCast->Text + ",'补新卡，旧卡卡号" + edcard->Text + "','" + edbircard->Text +"','"+DateToStr(dtpyouxiao->Date)+ "')";
	aq->Close();
	...
	try {
        ...
        ShowMsg(Handle ,"添加新会员失败，请检查输入信息是否正确！",3);
		return;
	}
