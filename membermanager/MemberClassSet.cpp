...
void __fastcall Tfrmmemclassmg::BtnDeleteClick(TObject *Sender)
{
	...
	else
	{
		...
		if (!quMemberType->IsEmpty()) {
                        ...
                        ShowMsg(Handle ,"该会员类型的会员信息存在，不能删除该会员类型！",3);
		}
		else
		{
...
void __fastcall Tfrmmemclassmg::BtnSaveClick(TObject *Sender)
{
    if (edTypeName->Text == "") {
                ...
                ShowMsg(Handle ,"会员类型名称不能为空！",3);
		return;
	}
	if (edTypeDiscount->Text == "") {

                ShowMsg(Handle ,"会员类型折扣不能为空！",3);
		return;
	}
	int discount;
	...
	if (discount > 100) {
        ...
        ShowMsg(Handle ,"会员类型折扣不能大于100！",3);
		return;
	}
	if (savemodle == 1) {
		Addsave();
