...
{
	AnsiString  card = "",name = "",sql = "select CUST_MemberInfo.ID,CUST_MemberInfo.CardSN,CUST_MemberInfo.CardID,CUST_MemberInfo.Name,CUST_MemberType.Type,CUST_MemberInfo.Balance, convert(varchar(10),CUST_MemberInfo.Birthday,23) as Birthday,CUST_MemberInfo.Sex,CUST_MemberInfo.CardType,CUST_MemberInfo.PassWord,case CUST_MemberInfo.Sex when 0 then '男' when 1 then '女' end as Sexname,CUST_MemberInfo.Tel,CUST_MemberInfo.Mobile,CUST_MemberInfo.Address,CUST_MemberInfo.PostalCode,CUST_MemberInfo.TotalConsumption,CUST_MemberInfo.MemberType,CUST_MemberInfo.Memberdot,case CUST_MemberInfo.Available when 0 then '无效'  else '有效' end as aviable,CUST_MemberInfo.CardCast from CUST_MemberInfo inner join CUST_MemberType on CUST_MemberInfo.MemberType = CUST_MemberType.IDType ";
	  ...
	  if (card != "") {
			if (name != "") {
				  sql = sql + "where CardID like '%" + card + "%' and Name like '%" + name + "%'";//+" and CUST_MemberInfo.stgidex ="+IntToStr(stgid);
			}
			else {
				  sql = sql + "where CardID like '%" + card + "%'";// +" and CUST_MemberInfo.stgidex ="+IntToStr(stgid);
			}
	  }
	  else
	  {
          if (name != "") {
				  sql = sql + "where name like '%"  + name + "%'";//+" and CUST_MemberInfo.stgidex ="+IntToStr(stgid);
			}
	  }
	  quMemberInfo->Close();
	  quMemberInfo->SQL->Clear();
