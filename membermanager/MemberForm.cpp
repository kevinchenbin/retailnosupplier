	...
	: TForm(Owner)
{
  ...
  sql = "select CUST_MemberInfo.ID,case cardtype when 1 then '赠卡' when 0 then '购卡' when NULL then '' end as cardtypestr,CUST_MemberInfo.CardSN,CUST_MemberInfo.CardID,CUST_MemberInfo.Name,CUST_MemberType.Type,CUST_MemberInfo.CardType,CUST_MemberInfo.PassWord,CUST_MemberInfo.bircard,"
		"CUST_MemberInfo.Balance,CUST_MemberInfo.Birthday,CUST_MemberInfo.Sex,case CUST_MemberInfo.Sex when 0 then '男' when 1 then '女' end as Sexname,CUST_MemberInfo.Tel,CUST_MemberInfo.CardCast,CUST_MemberInfo.youxiaodate,CUST_MemberInfo.Memberdot,"
...
{
	  AnsiString  sql = "select RANK() OVER(order by CUST_MemberInfo.ID ) as xuhao,case cardtype when 1 then '赠卡' when 0 then '购卡' when NULL then '' end as cardtypestr,CUST_MemberInfo.ID,CUST_MemberInfo.CardSN,CUST_MemberInfo.bircard,CUST_MemberInfo.CardID,CUST_MemberInfo.Name,CUST_MemberType.Type,CUST_MemberInfo.Balance, CUST_MemberInfo.Birthday,convert(varchar(12),CUST_MemberInfo.createdate,23) as createdate,CUST_MemberInfo.Sex,CUST_MemberInfo.CardType,CUST_MemberInfo.PassWord,case CUST_MemberInfo.Sex when 0 then '男' when 1 then '女' end as Sexname,CUST_MemberInfo.Tel,CUST_MemberInfo.Mobile,CUST_MemberInfo.Address,CUST_MemberInfo.PostalCode,CUST_MemberInfo.TotalConsumption,CUST_MemberInfo.MemberType,CUST_MemberInfo.Memberdot,CUST_MemberInfo.youxiaodate,case CUST_MemberInfo.Available when 0 then '无效'  else '有效' end as aviable,CUST_MemberInfo.CardCast from CUST_MemberInfo inner join CUST_MemberType on CUST_MemberInfo.MemberType = CUST_MemberType.IDType where 1 = 1 ";
...
void __fastcall TfrmMember::btUpdateClick(TObject *Sender)
{
   ...
   int c_update = 2;     //修改会员
   if (quMemberInfo->FieldByName("Name")->AsString == "0000") {
		ShowMsg(Handle ,"默认会员不能修改！",3);
		return;
   }
       //	if (quMemberInfo->FieldByName("aviable")->AsString == "无效") {
	   //	ShowMsg(Handle ,"该会员卡已退卡，不能修改！",3);
	   //	return;
  // }
   String *m_MemberInfo;
   m_MemberInfo = new String[17];
   ...
   m_MemberInfo[15] = quMemberInfo->FieldByName("youxiaodate")->AsString ;
    m_MemberInfo[16] = quMemberInfo->FieldByName("memberdot")->AsString ;
...
void __fastcall TfrmMember::btDeleteClick(TObject *Sender)
{
	...
	if (count>0) {
		ShowMsg(Handle ,"该会员有销售记录，不能删除！",3);
		return;
	}
	AnsiString sqlMemberRecord = "delete from CUST_MemberToprecord where CardID = '";
	...
	if(!quMemberInfo->IsEmpty())
	{
		...
		if (ShowMsg(Handle ,sdf.c_str() ,1)==1) {
			...
			{
				ShowMsg(Handle ,"此数据跟系统其他地方有关联，不能删除！",3);
			}
		}
	}
...
void __fastcall TfrmMember::addvalueClick(TObject *Sender)
{
	...
	if (quMemberInfo->FieldByName("Name")->AsString == "普通会员") {
                ...
                ShowMsg(Handle ,"普通会员不能充值！",3);
		return;
   }
    if (quMemberInfo->FieldByName("aviable")->AsString == "无效") {

                ShowMsg(Handle ,"该会员卡已退卡，不能充值！",3);
		return;
	}
...
bool __fastcall TfrmMember::DbgridToExcel(TDBGrid* dbg)
{
	...
	savedlg->FileName ="会员资料";// StringReplace(cbclient->Text.Trim() +dbeditdanhao->Text ,"\\","-",TReplaceFlags()<<rfReplaceAll) + "入库单";
	if (savedlg->Execute())
	{
		String DBPath,Name;
...
//  在Excel中成字符串显示
	...
	try {
      ...
      MessageBox(0, "Excel导出结束!","Excel导出", MB_OK | MB_ICONINFORMATION);
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmMember::BtnErrorClick(TObject *Sender)
{
	...
	if (quMemberInfo->FieldByName("Name")->AsString == "普通会员") {
                ...
                ShowMsg(Handle ,"普通会员不能退卡，不能修改！",3);
		return;
	}
	if (quMemberInfo->Active == true && quMemberInfo->RecordCount > 0) {
    	if (quMemberInfo->FieldByName("aviable")->AsString == "无效") {

                        ShowMsg(Handle ,"该会员卡已经作废，不能修改！",3);
			return;
		}
...
void __fastcall TfrmMember::BtnWordWrapClick(TObject *Sender)
{
	...
	if (aq->IsEmpty() ) {
             ...
             ShowMsg(Handle ,"没设置积分兑换金额，不能兑换！！",3);
		return;
	}
	delete aq;
...
void __fastcall TfrmMember::BtnClearClick(TObject *Sender)
{
	...
	if (ShowMsg(Handle ,"确认清除会员积分？",1)==1) {
		try{
			...
			quMemberInfo->Active = false;
			quMemberInfo->Active = true;
                        ShowMsg(Handle ,"积分清零完成！",3);
		}
		catch(Exception &E)
		{
                        ShowMsg(Handle ,"积分清零失败！",3);
		}
	}
