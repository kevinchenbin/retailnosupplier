...
bool Tfrmsiglebook::shuzhi(AnsiString context)
 {
		...
		else
		{
			...
			sql = "select BS_BookCatalog.id,BS_BookCatalog.ISBN,BS_BookCatalog.Name,BS_BookCatalog.UserDefCode,Price,PressCount,BS_BookCatalog.Author,BS_PressInfo.AbbreviatedName,BS_BookType.Name AS typename,sum(A.amount) as amount "
					 ...
					 " where BS_BookCatalog.Unavailable = 1 ";
                         AnsiString SearchCont = edquery->Text.Trim();
			...
			switch (m_catalogSearchMode) {
				case 0:
					     ...
					     sql = sql + " and (BS_BookCatalog.ISBN like '%" + SearchCont  + "%' or BS_BookCatalog.Barcode like '%" + SearchCont  + "%') " ;
                                          }else
                                          {
	...
	switch (rg->ItemIndex ) {
		case 0://订单
			sql = "select distinct A.id, convert (nvarchar(10),B.HdTime,120) as HdTime,B.OrderNtCode AS Code,dbo.UF_BS_GetClientName(B.VendorID) as Name,"
					" A.Amount,A.Discount,A.FixedPrice,A.DiscountedPrice,A.SendAmount,A.UnsendAmount "
					" ,B.remarks from BS_OrderNote A left join BS_OrderNoteHeader B on A.OrderNtHeaderID = B.id where A.state = 0 and A.BkcatalogID = " + IntToStr(bkcatalogid) + " and B.StgID = " + IntToStr(fstgid);
			if (chstart->Checked ) {
			...
			aq1->SQL->Add(sql);
			aq1->Open();
			dg1->Columns->Items[2]->Title->Caption = "客户名称";
		...
		case 1: //采购
			sql = "select distinct A.id, convert (nvarchar(10),B.HdTime,120) as HdTime,B.ProcureNtCode AS Code,CUST_CustomerInfo.Name as Name,"
					" A.Amount,A.Discount,A.FixedPrice,A.DiscountedPrice,A.RecAmount,A.UnrecAmount,B.remarks "
			...
			aq1->SQL->Add(sql);
			aq1->Open();
			dg1->Columns->Items[2]->Title->Caption = "供应商名称";
			dg1->Columns->Items[3]->Title->Caption = "数量";
		...
		case 2://入库
			if (ChangeDisplay) {
				sql = "select distinct A.id, convert (nvarchar(10),B.HdTime,120) as HdTime,B.StgNtCodeStr AS Code,CUST_CustomerInfo.Name as Name,"
					" A.Amount,A.Discount,A.FixedPrice,A.DiscountedPrice,B.remarks "
		...
		case 4://零售
			sql = "select distinct A.id, convert (nvarchar(10),B.HdTime,120) as HdTime,B.RetailNtCode AS Code,CUST_MemberInfo.Name as Name,"
					" A.Amount,A.Discount,A.FixedPrice,A.DiscountedPrice ,'' remarks "
			...
			aq1->SQL->Add(sql);
			aq1->Open();
			dg1->Columns->Items[2]->Title->Caption = "会员名称";
			dg1->Columns->Items[3]->Title->Caption = "数量";
			dg1->Columns->Items[7]->Visible = false;
			dg1->Columns->Items[8]->Visible = false;
                        dg1->Columns->Items[9]->Visible = false;
			dg1->Visible = true;
			dg2->Visible = false;
                        dg3->Visible = false;
		...
		break;
                ...
                case 8://省代采购
                  ...
                  sql = sql +" A.RecAmount ,A.caigoudiscount as Discount,A.UnrecAmount , d.name as opname from BS_DaixiaoHeader A join BS_DaixiaoNote B on a.id = b.daixiaoheadid ";
...
void __fastcall Tfrmsiglebook::dg2TitleClick(TColumnEh *Column)
{
	if (aq2->IsEmpty() ) {
		return;
	}
...
void __fastcall Tfrmsiglebook::FormShow(TObject *Sender)
{
if (0!=version) {
