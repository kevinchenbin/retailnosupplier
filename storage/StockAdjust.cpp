		...
		if (mrOk == results)
		{
			if (frm->SelectAll) {
				 for (int i = 0; i < frm->lengthint  ; i++) {
					AddNote(0,frm->BkcatalogiD[i],frm->stkbkid[i],frm->skAmount[i]);
				 }
			}else
			{
			   frm->GetSelectResult(bkinfoid,catalogID);
			   AddNote(0,catalogID,frm->BKinfoid,frm->Amount);
			   editadjust->SetFocus();
			}
		}
...
void TfrmStockAdjust::AddNote(int supplierID,int CatalogId,int bkinfoid,int amount)
{
   ...
   //	locvalues[0]   =   Variant(IntToStr(supplierID));
	locvalues[0]   =   Variant(IntToStr(CatalogId));
   //	if ( !aqdetail->Locate("bkcatalogID",VarArrayOf(locvalues,1),Opts))
  //	{
    	String sql;
		sql = "exec USP_BS_StockAdjustAddNote ";
		sql = sql +editnum->Text + ","  + IntToStr(CatalogId) + "," + IntToStr(bkinfoid) + "," + IntToStr(amount) ;
		ExecSQL(sql);
