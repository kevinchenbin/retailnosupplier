	...
	LNodeData *Ldata;
		  ...
		  if (OperModal == 2) {        //修改模式
				if ((tvlocal->Selected == NULL)||(tvlocal->Selected->Parent == NULL)) {
                                 ...
                                 ShowMsg(Handle ,"没有选择库位！",3);
			 }else
			{
				if (tvlocal->Selected == "默认库位") {


                                  ShowMsg(Handle ,"不能修改默认库位！",3);
				  return;
				}
...
void __fastcall Tfrmstorelocal::BtnDeleteClick(TObject *Sender)
{
  AnsiString sql;
	   if ((tvlocal->Selected != NULL)&&(tvlocal->Selected->Parent != NULL)) {
			...
			if (ShowMsg(Handle ,"确认要删除吗?",1)==1)     //有问题
			{
			if (tvlocal->Selected->Text == "默认库位") {
				 ShowMsg(this->Handle,"默认库位，不能删除！",3);
				 return;
			}
