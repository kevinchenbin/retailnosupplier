...
void Tfrmalarm::Filter()
{
	...
	sql = "select cast(rank() over (order by A.id) as nvarchar(10)) as xuhao,C.Name as supplier,B.ISBN,B.name,B.author,B.price,A.amount,F.maxnum,F.minnum,D.Name as typename,E.AbbreviatedName,"
			" case when A.amount-F.maxnum > 0 then A.amount-F.maxnum else 0 end as c, "
			" case when F.minnum - a.amount > 0 then F.minnum - a.amount else 0 end as d from STK_BookInfo A left join BS_BookCatalog B "
			" on A.bkcatalogID = B.ID  "
			" left join stk_bookinfobk F on A.id = F.bkinfoid "
			...
			" where  A.stgid = " + IntToStr(fstgid) + " and (A.amount > F.maxnum or A.amount < F.minnum) and (F.maxnum <> 0 or F.minnum <> 0)";
	if (chchaochu->Checked ) {
		if (edchaochumin->Text != "" && edchaochumin->Text != "0") {
			filterstr = " and A.amount-F.maxnum >= " + edchaochumin->Text ;
		}
		if (edchaochumax->Text != "" && edchaochumax->Text != "0") {
			filterstr = filterstr + " and A.amount-F.maxnum <= " + edchaochumax->Text ;
		}
	}
	if (chduanque->Checked ) {
		if (edduanquemin->Text != "" && edduanquemin->Text != "0") {
			filterstr = filterstr + " and F.minnum - a.amount >= " + edduanquemin->Text ;
		}
		if (edduanquemax->Text != "" && edduanquemax->Text != "0") {
			filterstr = filterstr + " and F.minnum - a.amount <= " + edduanquemax->Text ;
		}
	}
	...
	sql = sql + " union "
			...
			" sum(case when A.amount-F.maxnum > 0 then A.amount-F.maxnum else 0 end) as c,"
			" sum(case when F.minnum - a.amount > 0 then F.minnum - a.amount else 0 end) as d from STK_BookInfo A left join BS_BookCatalog B "
			" on A.bkcatalogID = B.ID "
			" left join stk_bookinfobk F on A.id = F.bkinfoid "
			" left join CUST_CustomerInfo C on A.supplierid = C.ID "
			" left join BS_BookType D ON B.smallBookTypeID =	D.id "
			" left join BS_PressInfo E on B.PressID = E.id "
			" where (A.amount > F.maxnum or A.amount < F.minnum) "
			" and (F.maxnum <> 0 or F.minnum <> 0) " + filterstr;
	aquery->Close();
	aquery->SQL->Clear();
	aquery->SQL->Add(sql);
	aquery->Open();
...
void __fastcall Tfrmalarm::edqueryKeyPress(TObject *Sender, wchar_t &Key)
{
	if (Key == '\r') {
		AnsiString sql;
		sql = "select ID,Name from CUST_CustomerInfo where type = 1 and customerstate=1 and stgid="+IntToStr(fstgid)+" and name like '%" + edquery->Text.Trim() + "%'";
		query->Close();
		query->SQL->Clear();
		query->SQL->Add(sql);
		query->Open();
