...
void __fastcall Tfrmdiaobo::dbnumamount1Exit(TObject *Sender)
{
	...
	if (aquery->FieldByName("Amount")->AsInteger > aquery->FieldByName("bkamount")->AsInteger && dsetNtHeader->FieldByName("state")->AsInteger == 0 ) {
		MessageBox(0,"配送数量大于当前库存数量！","配送" ,MB_ICONEXCLAMATION);
		dbnumamount->SetFocus();
		return;
	}
	UpdateNote();
