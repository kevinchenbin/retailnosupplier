...
void __stdcall SetUpStorageInfo(TApplication* app, TADOConnection* con,int userid,int storageid,bool master,int version)
{
	Application = app;
	...
	frm->Init(con);
        if (0==version) {
         frm->rztoolshendai->Visible = false;
         frm->dbGridCust->Columns->Items[3]->Visible=false;
        }
