...
void TfrmWsaleDiscount::View(int tag)
{
	...
	switch (tag ) {
			case 1:        //全部
				...
				Labelname->Visible = false;
				cbtype->Visible = false;
				sql = "select b.name as storagename,'全部' as name,a.alldot as dot from BS_WsaletypeDiscountInfo a join SYS_StorageInfo b on a.stgid=b.id where type = 1  and stgid = " + IntToStr(stgid);
				query->Close();
				query->SQL->Clear();
				query->SQL->Add(sql);
				query->Open();
                                DBGrid1->Columns->Items[1]->Title->Caption  ="全部";
			...
			case 2:        //版别
				...
				Labelname->Caption = "出版社";
                                DBGrid1->Columns->Items[1]->Title->Caption  ="出版社";
			...
			case 3:        //类别
				...
				Labelname->Caption = "类别";
                                 DBGrid1->Columns->Items[1]->Title->Caption  ="类别";
			...
			case 4:        //供应商
				...
				Labelname->Caption = "供应商";
                                    DBGrid1->Columns->Items[1]->Title->Caption  ="供应商";
...
void __fastcall TfrmWsaleDiscount::BtnSaveClick(TObject *Sender)
{
	...
	if (discount < 0) {
                ...
                ShowMsg(Handle ,"附加折扣不能小于0！",3);
		return;
	}
	if (discount > 100) {

                 ShowMsg(Handle ,"附加折扣不能大于100！",3);
		return;
	}
	int stgid;
	try {
		...
		try {
			...
			switch (type) {
				case 2:
                                         ...
                                         ShowMsg(Handle ,"请选择正确版别！",3);
				break;
				case 3:

                                        ShowMsg(Handle ,"请选择正确类别！",3);
				break;
				case 4:

                                        ShowMsg(Handle ,"请选择正确供应商！",3);
