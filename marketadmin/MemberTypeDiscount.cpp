...
void __fastcall TfrmMemberDiscount::DBGrid1TitleClick(TColumn *Column)
{
	if (aq->IsEmpty() ) {
		return;
	}
...
void __fastcall TfrmMemberDiscount::dbgdiscountTitleClick(TColumn *Column)
{
	if (querydetail->IsEmpty() ) {
		return;
	}
...
void __fastcall TfrmMemberDiscount::N7Click(TObject *Sender)
{
      //
      BtnSave->Click();
}
