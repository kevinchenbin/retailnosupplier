	...
	: TForm(Owner)
{
	...
	if (Type == 1) {
		...
		Label21->Caption = "付款单";
		cbjieshuantype->AddItem("付款退款",(TObject*)1);
		cbjieshuantype->AddItem("余额付款",(TObject*)2);
		cbjieshuantype->AddItem("预付款",(TObject*)3);
		cbjieshuantype->AddItem("往来对冲",(TObject*)4);
		cbjieshuantype->AddItem("余额退款",(TObject*)5);
	...
	}else
	{
		...
		Label21->Caption = "收款单";
		cbjieshuantype->AddItem("收款退款",(TObject*)1);
		cbjieshuantype->AddItem("余额收款",(TObject*)2);
		cbjieshuantype->AddItem("预收款",(TObject*)3);
		cbjieshuantype->AddItem("往来对冲",(TObject*)4);
	  	cbjieshuantype->AddItem("余额退款",(TObject*)5);
		Label12->Caption = "预收款";
...
{
	if (ShowMsg(this->Handle,"确认要添加新单吗？",3) == 1) {
		Enable(true);
		save = false;
		BtnOK->Enabled = true;
...
void __fastcall Tfrmbalance::BtnSaveClick(TObject *Sender)
{
	if (!save) {
		if (Type == 1 && cbselect->Text == "") {
			ShowMsg(Handle ,"请选择结算供应商！",3);
			return;
		}
		if (Type == 2 && cbselect->Text == "") {
			ShowMsg(Handle ,"请选择结算客户！",3);
			return;
		}
		if (cbjieshuantype->Text  == "") {
			ShowMsg(Handle ,"结算类型不能为空！",3);
			return;
		}
		if (cbmethod->Text == "") {
			ShowMsg(Handle ,"结算方式不能为空！",3);
			return;
		}
...
void __fastcall Tfrmbalance::editmoneyKeyPress(TObject *Sender, wchar_t &Key)
{
	...
	if (editstate->Text =="已审核") {
		ShowMsg(Handle ,"该张单子已审核，不能再结算！",0);
		return;
	}
	try {
		...
		ShowMsg(Handle ,"请输入正确的金额，不能再结算！",0);
		return;
	}
	if (num == 0) {
		return;
	}else
	{
		...
		if (Disc == "0.0") {
			if ((num - hukuan )>0.1 && (cbjieshuantype->ItemIndex != 2 && cbjieshuantype->ItemIndex != 4)) {
				ShowMsg(Handle ,"付款金额过多,请重新付款，不能再结算！",3);
				editmoney->Text = "0.00";
				editmoney->SetFocus();
				return;
			}
...
void __fastcall Tfrmbalance::BtnOKClick(TObject *Sender)
{
	...
	aquery->Open();
     //	if (aquery->RecordCount > 0) {
       //		ShowMsg(Handle ,"本次结款金额有大于未结金额，请查正后再审核！",0);
      //  DisplayDetail(editnum->Text);
     //		return;
     //	}

	if(ShowMsg(Handle ,"确认要审核该张单子吗?",1)==1 )
	{
		shenhe();
...
void __fastcall Tfrmbalance::BtnDeleteClick(TObject *Sender)
{
	...
	if (cbjieshuantype->ItemIndex == 3) {
		if( ShowMsg(Handle ,"结算类型为往来对冲时，无法针对一行删除，如果确定将会删除所有记录",1)==1 )
		{
	...
	{
		if( ShowMsg(Handle ,"确认删除结款记录吗？",1)==1 )
		{
...
void __fastcall Tfrmbalance::BtnCancelClick(TObject *Sender)
{
	...
	if(ShowMsg(Handle ,"确认要反审核该张单子吗?",1)==1 )
	{
		String sql;
		AnsiString zryuhu;
		editweijiedan->Text = StringReplace(editweijiedan->Text,",","",TReplaceFlags()<<rfReplaceAll);
		...
		else
		{
			...
			while(!aqdg->Eof)
			{
				if (Totalyinhu > 0) {
                                        ...
                                        if (aqdg1->FieldByName("nochecked")->AsFloat>0) {    //如果是负数，则不管直接更新thistime
					...
					if (shijie - aqdg1->FieldByName("nochecked")->AsFloat >= 0)
					{
						...
						if (shijie == 0) {
                            break;
                                                ...
                                                if (aqdg1->FieldByName("nochecked")->AsFloat<0 ) {   //如果小于０
                                                  	aqdg->Edit();
...
void __fastcall Tfrmbalance::cbjieshuantypeExit(TObject *Sender)
{
	...
	if (StrToFloat(yue) == 0.00 && (cbjieshuantype->ItemIndex == 1 || cbjieshuantype->ItemIndex == 4)) {
                ...
                ShowMsg(Handle ,"没有余额，不能用余额进行结款！",0);
                return;
		cbjieshuantype->ItemIndex = -1;
		btduicong->Enabled = true;
		spselect->Enabled = true;
	...
	if (StrToFloat(weijie) == 0.00 && StrToFloat(yue) == 0 && cbjieshuantype->ItemIndex != 2) {
		if (Type == 1) {
                        ...
                        ShowMsg(Handle ,"无需要结算的单据，只能选择预付款！",0);
		}
		else
		{

                        ShowMsg(Handle ,"无需要结算的单据，只能选择预收款!",0);
		}
		cbjieshuantype->ItemIndex = 2;
		btduicong->Enabled = false;
		spselect->Enabled = false;
		...
		if (Type == 1) {
                        ...
                        ShowMsg(Handle ,"无需要结算的单据，只能选择预付款或余额退款!",0);
		}
		else
		{
                        ShowMsg(Handle ,"无需要结算的单据，只能选择预收款或余额退款!",0);
		}
		...
		if (aquery->IsEmpty() ) {
                        ...
                        ShowMsg(Handle ,"无对冲单据，不能使用往来对冲!",3);
			cbjieshuantype->ItemIndex = -1;
			btduicong->Enabled = true;
			spselect->Enabled = true;
			sbautojieshuan->Enabled = true;
