...
void __fastcall Tfrmclientwanglai::BtnViewClick(TObject *Sender)
{
	...
	if (Client== 1) { //供应商
		...
		sql = sql + " A.TotalAmount as '数量',A.TotalFixedPrice as '码洋',A.TotalDiscountedPrice as '实洋',A.AddCosts as '附加费用',A.AddCosts + A.TotalDiscountedPrice as totalyingfu, A.checked  as '已结',A.nochecked as '未结',A.duifangmoney as '对方账务',A.hongcong as '红冲', "
	...
	while(!dg->DataSource ->DataSet->Eof)
	{
		...
		for(int j=1;j<dg->Columns ->Count+1  ;j++)
		{
			if (dg->Columns->Items[j-1]->Visible == true) {
				if (dg->Columns ->Items [j-1]->FieldName == "单号") {
