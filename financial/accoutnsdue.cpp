...
void __fastcall TAccountDoe::FormShow(TObject *Sender)
{
	AnsiString sql ;
	if (type == 1) {
		sql = "select ID,Name from CUST_CustomerInfo where Type = 1  and stgid="+IntToStr(fstgid)+" and name <> ''  and customerstate=1 order by name";
	}
	else
	{
		sql = "select ID,Name from CUST_CustomerInfo where Type in(2,3) and stgid="+IntToStr(fstgid)+" and name <> ''  and customerstate=1 order by name";
	}
	aqcustom->Close();
	aqcustom->SQL->Clear();
...
void __fastcall TAccountDoe::BtnViewClick(TObject *Sender)
{
	...
	if (type == 1) {
		sql = "select cast(rank() over(order by CUST_CustomerInfo.ID) as nvarchar(10)) as xuhao,sum(BS_StorageNoteHeader.TotalFixedPrice) as sumFixedPrice,sum(BS_StorageNoteHeader.TotalDiscountedPrice) as sumDiscountedPrice, "
		 ...
		 sqlwhere = " where CUST_CustomerInfo.Type = 1 and CUST_CustomerInfo.stgid="+IntToStr(fstgid);
	}
	else
	{
		...
		{
			sql = sql + " 0.00 as profite,0.00 as cbprice, ";
		}
		...
		sqlwhere = " where CUST_CustomerInfo.Type in(2,3) ";
	}
...
bool __fastcall TAccountDoe::DbgridToExcel(TDBGridEh* dbg)
{
	...
	{
		if (dbg->Columns->Items[1]->Visible == true) {
			t1++;
