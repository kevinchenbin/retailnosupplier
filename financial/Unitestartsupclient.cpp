...
//---------------------------------------------------------------------------
__fastcall Tfrmsupplierandclient::Tfrmsupplierandclient(TComponent* Owner,TADOConnection *cn,int type,int stgid)
	: TForm(Owner)
{
   	fcon = cn;
...
void Tfrmsupplierandclient::ReadyData(int type)
{
	...
	aq->Connection = fcon;
	if (type == 1) {
		sqlstr = "select ID,Name from CUST_CustomerInfo where type = 1 and customerstate=1 and stgid="+IntToStr(fstgid)+" and name <> '' order by name";
	}
	else
	{
		sqlstr = "select ID,Name from CUST_CustomerInfo where type in(2,3) and customerstate=1 and stgid ="+IntToStr(fstgid)+" order by name";
	}
...
void __fastcall Tfrmsupplierandclient::editmoneyKeyPress(TObject *Sender, wchar_t &Key)
{
	...
	if (Key==VK_RETURN) {
		...
		if (ShowMsg(Handle ,"确认修改期初值吗！",1) == 1) {

			try {
				mon =  StrToFloat(editmoney->Text) ;
			} catch (...) {

                                ShowMsg(Handle ,"请输入正确的期初金额！",3);
				return;
			}
			if (labeltitle->Tag == 0 ) {   //书店欠供应商
				if (Type == 1) {
...
void Tfrmsupplierandclient::Query()
{
	...
	if (Type==1) {
		...
		sqlwhere = " from  CUST_CustomerInfo A join CUST_CustomerStartMoney B on A.ID = B.customerID where A.Type=1  and A.Name <> '' and A.stgid="+IntToStr(fstgid);
		if (cbselect->Text != "") {
		...
		sqlwhere = " from  CUST_CustomerInfo A join CUST_CustomerStartMoney B on A.ID = B.customerID where A.Name <> '' and Type in(2,3) and A.stgid="+IntToStr(fstgid);
		if (cbselect->Text != "") {
			sqlwhere = sqlwhere + " and A.Name = '" + cbselect->Text.Trim() + "'";
		}
	}
