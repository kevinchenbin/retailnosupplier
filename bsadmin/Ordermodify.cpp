...

{
	if (Key == '\r') {
		...
		aq->Connection = con;
		AnsiString sql;
		sql = "select ID,Name from CUST_CustomerInfo where type in(2,3) and name like '%" + edclient->Text.Trim() + "%'";
		aq->Close();
		aq->SQL->Clear();
		aq->SQL->Add(sql);
		aq->Open();
...
void Tfrmordermodify::DbgridToExcel()
{
	...
	aq2->EnableControls();
	aq1->EnableControls();
        ShowMsg(Handle ,"导出成功,请在指定的目录下注意查看！",3);
}
//---------------------------------------------------------------------------
...
void __fastcall Tfrmordermodify::cbclientChange(TObject *Sender)
{
	if (cbclient->Text != "") {
...
void __fastcall Tfrmordermodify::cbclientClick(TObject *Sender)
{
	try {
