...
void __fastcall Tfrmorderask::BtnViewClick(TObject *Sender)
{
	...
	sqldaochu = "select BS_BookCatalog.ISBN,BS_BookCatalog.Name as '书名',convert(nvarchar(10),BS_BookCatalog.Price) as '定价',BS_BookCatalog.Author as '作者',"
			" BS_PressInfo.AbbreviatedName as '出版社',BS_BookCatalog.PressCount as '版次',";
	if (chuseamount->Checked ) {
		sql = sql + " sum(BS_OrderNote.Amount) as caigouamount,";
		sqldaochu = sqldaochu + " sum(BS_OrderNote.Amount) as '采购数量',";
	...
	else
	{
		...
		sqldaochu = sqldaochu + " case when sum(BS_OrderNote.Amount)-Order_lock.usableamount > 0 then sum(BS_OrderNote.Amount)-Order_lock.usableamount else 0 end as '采购数量',";
	...
	sqldaochu = sqldaochu + " dbo.UF_BS_GetOrderCode(BS_OrderNote.BkcatalogID) as '订单号' ";
...
bool Tfrmorderask::DbgridToExcel()
{
	...
	if(FileExists(path))
	   if (DeleteFileA(path))
		...
		iFileHandle = FileCreate(path.c_str());
	temptext = "ISBN,书名,自编码,定价,作者,出版社,版次,采购数量,订单号";
	...
	while (!query->Eof)
	{
		if (query->Fields->Fields[9]->AsInteger > 0) {
			...
			else
			{
				...
				failename = query->FieldByName("AbbreviatedName")->AsAnsiString;
				if (failename == "") {
					path = DBPath + "出版社不详("+ storagename + DateToStr(Date()) + ").csv";
				}
				else
				{
...
bool Tfrmorderask::DbgridToExcel1()
{
	...
	while (!aqpress->Eof )
	{
		...
		else
		{
			...
			//path = DBPath + IntToStr(t2) +".xlsx";
			path = DBPath + "出版社不详("+ storagename + DateToStr(Date()) + ").xls";
		}
		if (FileExists(path) ) {
			try {
		...
		temptext = "传真日期：" + DateToStr(Date()) ;
		V.OlePropertyGet("Cells",2,1).OlePropertySet("Value",temptext .c_str() );
		R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",2,1),vSheet.OlePropertyGet("Cells",2,8)); //取得合并的区域
		...
		R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",3,4),vSheet.OlePropertyGet("Cells",3,8)); //取得合并的区域
		...
		R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",4,4),vSheet.OlePropertyGet("Cells",4,8)); //取得合并的区域
		...
		R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",5,4),vSheet.OlePropertyGet("Cells",5,8)); //取得合并的区域
		...
		R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",6,4),vSheet.OlePropertyGet("Cells",6,8)); //取得合并的区域
		...
		R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",7,1),vSheet.OlePropertyGet("Cells",7+h,8)); //取得合并的区域
		...
		/*R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",8,2),vSheet.OlePropertyGet("Cells",8+h,2)); //取得合并的区域
		R.OlePropertySet("WrapText", true);//设置指定区域中所有的单元格中的文本自动换行

		R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",8,8),vSheet.OlePropertyGet("Cells",8+h,8)); //取得合并的区域
		R.OlePropertySet("WrapText", true);//设置指定区域中所有的单元格中的文本自动换行
		*/
		if (h > 18) {
			R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",3,1),vSheet.OlePropertyGet("Cells",7+h,8)); //取得合并的区域
		}
		else
		{
			R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",3,1),vSheet.OlePropertyGet("Cells",25,8)); //取得合并的区域
		}
		...
		//R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",3,1),vSheet.OlePropertyGet("Cells",24,1)); //取得合并的区域
		//R.OlePropertyGet("Borders",1).OlePropertySet("linestyle",1);
		V.OlePropertyGet("Rows",26).OleProcedure("Insert");
		temptext = "备注：以上书籍如有不符，请尽快来电回告，谢谢!" ;
		V.OlePropertyGet("Cells",26,1).OlePropertySet("Value",temptext .c_str() );
		R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",26,1),vSheet.OlePropertyGet("Cells",26,8)); //取得合并的区域

		R.OleProcedure("Merge");
		R.OlePropertyGet("Borders",2).OlePropertySet("linestyle",1);

		V.OlePropertyGet("Rows",27).OleProcedure("Insert");
		temptext = "如发快件请尽快传真快件票!!!" ;
		V.OlePropertyGet("Cells",27,1).OlePropertySet("Value",temptext .c_str() );
		R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",27,1),vSheet.OlePropertyGet("Cells",27,8)); //取得合并的区域
		...
		R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",28,1),vSheet.OlePropertyGet("Cells",28,8)); //取得合并的区域

		R.OleProcedure("Merge");
		R.OlePropertyGet("Borders",2).OlePropertySet("linestyle",1);


		/*temptext = "我公司新开票纳税信息如下：";
		V.OlePropertyGet("Cells",28,1).OlePropertySet("Value",temptext .c_str() );
		temptext = "名        称：重庆世家文化教育服务有限责任公司";
		V.OlePropertyGet("Cells",29,1).OlePropertySet("Value",temptext .c_str() );
		temptext = "纳税人识别号：500112736590015";
		V.OlePropertyGet("Cells",30,1).OlePropertySet("Value",temptext .c_str() );
		temptext = "地 址、电 话：重庆市渝北区加州路38号商住楼3楼 023-62905966";
		V.OlePropertyGet("Cells",31,1).OlePropertySet("Value",temptext .c_str() );
		temptext = "开户行及账号：中国民生银行重庆加州支行 1108014210008385";
		V.OlePropertyGet("Cells",32,1).OlePropertySet("Value",temptext .c_str() );
		temptext = "收货地址：重庆南岸区学府大道37号重庆教科院巴蜀实验学校";
		V.OlePropertyGet("Cells",33,1).OlePropertySet("Value",temptext .c_str() ); */

		V.OlePropertyGet("Rows",28).OleProcedure("Insert");
		V.OlePropertyGet("Rows",28).OleProcedure("Insert");
		V.OlePropertyGet("Rows",28).OleProcedure("Insert");
		R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",28,1),vSheet.OlePropertyGet("Cells",31,8)); //取得合并的区域

		R.OleProcedure("Merge");
		R.OlePropertyGet("Cells").OlePropertyGet("Font").OlePropertySet("Bold",false);
		temptext = "我公司新开票纳税信息如下：\n"
				"名        称：重庆世家文化教育服务有限责任公司\n"
				"纳税人识别号：500112736590015\n"
				"地 址、电 话：重庆市渝北区加州路38号商住楼3楼 023-62905966\n"
				"开户行及账号：中国民生银行重庆加州支行 1108014210008385\n"
				"收货地址：重庆南岸区学府大道37号重庆教科院巴蜀实验学校";
		V.OlePropertyGet("Cells",28,1).OlePropertySet("Value",temptext .c_str() );
		...
		if (h > 18) {
			R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",3,1),vSheet.OlePropertyGet("Cells",7+h,1)); //取得合并的区域
		}
		else
		{
			R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",3,1),vSheet.OlePropertyGet("Cells",31,1)); //取得合并的区域
		}
		...
		R = vSheet.OlePropertyGet("Range",vSheet.OlePropertyGet("Cells",1,1),vSheet.OlePropertyGet("Cells",1,8)); //取得合并的区域
		R.OleProcedure("Merge");
		vSheet.OlePropertyGet("Cells",1,1).OlePropertyGet("Font").OlePropertySet("Name","宋体");   //字体
		R.OlePropertyGet("Cells").OlePropertyGet("Font").OlePropertySet("Bold",true);
