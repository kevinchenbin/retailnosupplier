...
void __fastcall Tfrmprintset::tvfanganClick(TObject *Sender)
{
	...
	AnsiString sql;
	sql = "select sys_printdetail.*,sys_printhead.reportname from sys_printdetail join sys_printhead on sys_printdetail.headid = sys_printhead.id where sys_printhead.modle = " + IntToStr(fmodle) + " and sys_printhead.stgid = " + IntToStr(fstgid) + " and sys_printhead.typename = '" + tvfangan->Selected->Text.Trim()  + "' order by id asc ";
	query->Close();
	query->SQL->Clear();
	query->SQL->Add(sql);
	query->Open();
	DBGridEh1->Enabled = false;
	edname->Enabled = false;
	cbreport->Text = query->FieldByName("reportname")->AsAnsiString ;
	modle = 0;
}
//---------------------------------------------------------------------------
