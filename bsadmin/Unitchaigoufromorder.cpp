...
{
	Tfrmcaigouorderselect * frm = new Tfrmcaigouorderselect(Application,linfo.con,linfo.storageID,0,0);
	frm->ShowModal();
	delete frm;
...
void __fastcall Tfrmchaigoufromorder::SpeedButton2Click(TObject *Sender)
{
	...
	Tfrmfeizhengshicai *frm = new Tfrmfeizhengshicai(Application,m_con,0,m_storageID);
	//frm->Show();
	if (frm->ShowModal() == mrOk ) {
...
void __fastcall Tfrmchaigoufromorder::SpeedButton3Click(TObject *Sender)
{
	...
	delete frm;
   	sql = "delete BS_Tmporder where modle = 1";
	aq->Close();
	aq->SQL->Clear();
	aq->SQL->Add(sql);
	aq->ExecSQL();
