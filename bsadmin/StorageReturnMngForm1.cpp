...
void TfrmStorageReturnMng1::AddNtHeader()//添加单头
{
...
void TfrmStorageReturnMng1::QryNtHeader(String code, bool qrybkstack)//查询单头
{
	spQryNtHeader->Active = false;
	spQryNtHeader1->Active = false;
	scroll = 1;
...
void TfrmStorageReturnMng1::QryNtHeader(bool qrybkstack)//查询单头
{
	//保存查询之前的当前活动记录ID
	int noteID = 0;
	if (dsetNote->Active && dsetNote->RecordCount >= 1)
...
void TfrmStorageReturnMng1::QryNtHeader(int id, bool qrybkstack)//查询单头
{
	//查询单头
	spQryNtHeader1->Active = false;
	spQryNtHeader->Active = false;
...
void TfrmStorageReturnMng1::DelNtHeader()//删除单头
{
//iu
}

void TfrmStorageReturnMng1::UpdateNtHeader()//更新单头
{
	if (dsetNtHeader->Active && dsetNtHeader->RecordCount >= 1)
	{
...
bool TfrmStorageReturnMng1::AddNote(int supplierID, int catalogID)//添加单
{
	bool result = false;
	if (dsetNtHeader->Active && dsetNtHeader->RecordCount >= 1 )
	{
		if (rktuihuoguanli) {
			...
			if (query->IsEmpty() ) {
				...
				else
				{
					...
					msg = "该书在供应商：" + msg + "处有库存,请查证后退货！";
				}

				MessageBox(0,msg.c_str() ,"提示",MB_ICONEXCLAMATION);
				edtCatalog->SetFocus();
				return true;
			}
		}
...
void TfrmStorageReturnMng1::UpdateNote()//更新单
{
	if (dsetNote->Active && dsetNote->RecordCount >= 1)
	{
...
void TfrmStorageReturnMng1::UpdateNote1()//更新单
{
	if (dsetNote->Active && dsetNote->RecordCount >= 1)
	{
		cmdUpdateNote->Parameters->ParamByName("@ID")->Value
...
void TfrmStorageReturnMng1::DelNote()//删除单
{
	if (dsetNote->Active && dsetNote->RecordCount >= 1)
	{
...
void __fastcall TfrmStorageReturnMng1::dbnbedtAmountExit(TObject *Sender)
{
	...
	if (rktuihuoguanli) {
		...
		if (dbnbedtAmount->IntValue > query->FieldByName("amount")->AsInteger - dsetNote->FieldByName("Amount")->AsInteger ) {
			MessageBox(0,"退货数量大于该供应商可退数量！","提示" ,MB_OK|MB_ICONWARNING);
			dbnbedtAmount->SetFocus();
			return;
		}
		if (dbnbedtAmount->IntValue > RzDBNumericEdit1->IntValue - dsetNote->FieldByName("Amount")->AsInteger ) {
			MessageBox(0,"退货数量大于该供应商库存数量！","提示" ,MB_OK|MB_ICONWARNING);
			dbnbedtAmount->SetFocus();
			return;
		}
	}
