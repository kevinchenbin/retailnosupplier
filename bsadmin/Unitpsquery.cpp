	...
	: TForm(Owner)
{
	...
	while (!aq->Eof )
	{
        ...
        if (!ftype) {    //ÇëÅäËÍ
           cbstate->Items->Add("´ýÉóºË");
            cbstate->Items->Add("´ý³ö¿â");
            cbstate->Items->Add("´ýÈë¿â");
            cbstate->Items->Add("ÒÑÈë¿â");
        }else
        {
                 cbstate->Items->Add("´ý³ö¿â");
                 cbstate->Items->Add("´ýÈë¿â");
...
{
	AnsiString sql;
	sql = "select BS_ZN_DiaoNoteHeader.ZNDiaoNtCode,BS_ZN_DiaoNoteHeader.StgNtCodeStr,BS_ZN_DiaoNoteHeader.HdTime,"
	...
	if (chcode->Checked && edcode->Text != "") {
		if (!ftype) {    //ÇëÅä
			sqlwhere = sqlwhere + " and BS_ZN_DiaoNoteHeader.StgNtCodeInt = " + edcode->Text;
		...
		}else //Ö÷Åä
		{
			...
			sqlwhere = sqlwhere + " and BS_ZN_DiaoNoteHeader.zStgNtCodeInt = " + edcode->Text;
		}
	}
        if (ckstate->Checked) {

             	if (!ftype) {
                      sqlwhere = sqlwhere + " and state ="+IntToStr(cbstate->ItemIndex);
                }else
                {
                     if (cbstate->ItemIndex==1) {
                      sqlwhere = sqlwhere + " and state =2";
                    }else if (cbstate->ItemIndex==1 ) {
                      sqlwhere = sqlwhere + " and state =2";
                   }else if (cbstate->ItemIndex==0) {
...
//---------------------------------------------------------------------------
void __fastcall Tfrmpsquery::edcodeKeyPress(TObject *Sender, wchar_t &Key)
{
...
void __fastcall Tfrmpsquery::cbstateChange(TObject *Sender)
{
//
  if (cbstate->Text !="") {
