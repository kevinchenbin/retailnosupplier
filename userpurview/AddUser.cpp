...
void __fastcall TfrmAddUser::FormShow(TObject *Sender)
{
	...
	if (Modle == 2) {     //修改用户
		...
		edbk->Text = query->FieldByName("bk")->AsString.Trim();
                editqq->Text =query->FieldByName("QQ")->AsString.Trim();

                if (!query->FieldByName("userstate")->AsBoolean) {
...
void __fastcall TfrmAddUser::BtnSaveClick(TObject *Sender)
{
	...
	if (aq->RecordCount > 0 && Modle == 1) {
                ShowMsg(Handle ,"该操作员已存在，请重新输入姓名！",3);
		edname->SetFocus();
		return;
	}
	/*if (cbusergroup->Text == "") {
	...
	if (Modle == 1) {
		...
		query->FieldByName("stgID")->AsInteger = (int)(cbstorge->Items->Objects[cbstorge->ItemIndex]);
                query->FieldByName("QQ")->AsAnsiString =  editqq->Text.Trim() ;
                if (cbstate->ItemIndex==1) {
                  query->FieldByName("userstate")->AsBoolean =false;
                }else {
                  query->FieldByName("userstate")->AsBoolean =true;
                }
	...
	if (Modle == 2) {
		...
		sql = sql + "',stgID = " + IntToStr((int)(cbstorge->Items->Objects[cbstorge->ItemIndex]));
                if (cbstate->ItemIndex==1) {
                    sql = sql + ",userstate = "+ IntToStr(0);
                }else
                {
                    sql = sql + ",userstate = "+ IntToStr(1);
                }
