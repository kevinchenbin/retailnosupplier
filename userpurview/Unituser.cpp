	...
	: TForm(Owner)
{
	...
	sql = "select A.*,case A.userstate when 1 then '在职' when 0 then '离职' end as state1 ,C.Name as storgename from SYS_User A left join SYS_StorageInfo C on A.stgID = C.ID where A.stgid = " + IntToStr(stgid) ;
	aquery->Close();
	aquery->SQL->Clear();
	aquery->SQL->Add(sql);
	aquery->Open();
...
void __fastcall Tfrmuser::BtnDeleteClick(TObject *Sender)
{
	...
	if (aquery->Active && aquery->RecordCount > 0) {
    	if (aquery->FieldByName("Name")->AsAnsiString == "Admin" ) {
                        ...
                        ShowMsg(Handle ,"默认系统管理员不能删除！",3);
			return;
		}
		if ( ShowMsg(Handle ,"确认删除该操作员?",1)==1) {
			try {
				...
				AddEvent(3,"操作员管理窗口",userid,fstgid,logmessage,cnn);
                                ShowMsg(Handle ,"该数据跟系统其他数据有关联！",3);
			}
		}
	}
}
