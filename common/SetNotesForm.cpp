...
void __fastcall TfrmSetNotes::FormShow(TObject *Sender)
{
 if ((numedtAmount->Visible)&&(numedtAmount->Enabled)) {
        numedtAmount->SelectAll();
        numedtAmount->SetFocus();
 }
 m_huiyuandis=100;
 m_lszddis=100;
...
void __fastcall TfrmSetNotes::huiyuandiscountChange(TObject *Sender)
{
   m_huiyuandis=fabs(huiyuandiscount->Value);
}
//---------------------------------------------------------------------------

void __fastcall TfrmSetNotes::lingkoudiscountChange(TObject *Sender)
{
m_lszddis=fabs(lingkoudiscount->Value);
}

bool TfrmSetNotes::Getlszddiscount(float&discount)
{
	bool ret = false;
	if (zdlingkoucheck->Checked)
	{
		discount = m_lszddis;
		ret = true;
	}
...
bool TfrmSetNotes::Gethuiyuandiscount(float&discount)
{

	   	bool ret = false;
	if (huiyuancheck->Checked)
	{
		discount = m_huiyuandis;
		ret = true;
	}
	return ret;
}
