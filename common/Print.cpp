...
void TfrmPrint::RetailPrint(String ID,TADOConnection* con,int stgid)
{
	...
	dsPrint->CommandText="select * from SYS_StorageInfo where id = " + IntToStr(stgid) ;
	dsPrint->Active=true;
	if (dsretail->Active) {
		dsretail->Active=false;
	}
	sql = "select rank() over (order by BS_RetailNote.id) as xuhao,len(BS_BookCatalog.Name) as lenthName,len(BS_BookCatalog.Name) as lenth,SYS_User.Name as operatorName,RecCash+RecCardMoney-TotalDiscountedPrice as backMoney,TotalFixedPrice,TotalDiscountedPrice,RecCash,RecCardMoney,poscard,youhuiquan,BS_BookCatalog.Name as bookName,BS_BookCatalog.ISBN as ISBN,BS_BookCatalog.Price as price ,RetailNtCode,HdTime as Time ,BS_RetailNote.Amount as Amount,DiscountedPrice ,FixedPrice,Discount,BS_RetailNoteHeader.TotalAmount,member.name as membername,member.memberdot,member.cardid,member.balance from BS_RetailNote left join BS_RetailNoteHeader on  RetailNtHeaderID=BS_RetailNoteHeader.id left join STK_BookInfo on BkInfoID=STK_BookInfo.id left join BS_BookCatalog on BkcatalogID=BS_BookCatalog.id left join SYS_User on SYS_User.id=OperatorID left join CUST_MemberInfo member on BS_RetailNoteHeader.memberid=member.id where BS_RetailNoteHeader.ID="+ID+" order by RetailNtCode";
	...
	else
	{
		...
		if (dsPrint->FieldByName("ShowRetailCardRec")->AsString==1) {
			s=s+"\n其中会员卡消费:"+FormatFloat(fromat,dsretail->FieldByName("RecCardMoney")->AsFloat);
		}
                if (dsPrint->FieldByName("printposcard")->AsString==1) {
			s=s+"\n其中银行卡消费:"+FormatFloat(fromat,dsretail->FieldByName("poscard")->AsFloat);
		}

                if (dsPrint->FieldByName("printname")->AsString==1) {
			s=s+"\n会员姓名:"+dsretail->FieldByName("membername")->AsString;
                       	s=s+"    卡号:"+dsretail->FieldByName("cardid")->AsString;
		}
                if (dsPrint->FieldByName("printcard")->AsString==1) {
                  	s=s+"\n卡内余额:"+FormatFloat(fromat,dsretail->FieldByName("balance")->AsFloat);
		}
                if (dsPrint->FieldByName("printjifen")->AsString==1) {
			s=s+"\n会员积分:"+dsretail->FieldByName("memberdot")->AsString;
		}
                if (dsPrint->FieldByName("printyouhuiquan")->AsString==1) {
			s=s+"\n优惠券消费金额:"+FormatFloat(fromat,dsretail->FieldByName("youhuiquan")->AsFloat);
		}
		...
		}else
		{
			...
			{
				MessageBox(0,"1此通讯口已被占用或损坏！","零售" ,MB_ICONEXCLAMATION);
				m_hComm=NULL;
				return ;
			}
			...
			if (m_hComm==INVALID_HANDLE_VALUE)
			{
				MessageBox(0,"2此通讯口已被占用或损坏！","零售" ,MB_ICONEXCLAMATION);
				m_hComm=NULL;
				return ;
			}
			comtimeout.ReadIntervalTimeout =10;
...
void TfrmPrint::moneyboxopen()
{
			  ...
			  TRaveMemo *RaveMemo;
				...
				RvSystem1->SystemSetups >>ssAllowPrinterSetup;
				RvSystem1->SystemSetups >>ssAllowDestFile;
			RvSystem1->DefaultDest   = rdPrinter ;//  rdPreview;//
						RvSystem1->SystemSetups<<ssAllowDestPrinter;
					   prjretail->ProjectFile  = ExtractFilePath(Application->ExeName)+"openmonybox.rav";
					   prjretail->Open();
					   RavePage = (TRavePage *)prjretail->ProjMan->FindRaveComponent("Report1.Page1",NULL);
						RaveMemo = (TRaveMemo *)prjretail->ProjMan->FindRaveComponent("Memo1",RavePage);
			RaveMemo->Text="";
...
void TfrmPrint::RetailjiaojiePrint(String userid,TADOConnection* con,int stgid)
{
		 ...
		 dsPrint->Active=true;
                 printusb = dsPrint->FieldByName("retailprintcom")->AsString;
		 ...
		 s = s + "零售码洋：" + dsjiaojieretail->FieldByName("TotalFixedPrice")->AsString + "\n";
		 s = s + "零售实洋：" + dsjiaojieretail->FieldByName("TotalDiscountedPrice")->AsString + "\n";
